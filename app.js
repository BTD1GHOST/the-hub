...
window.openComposer = function () {
  ...
  if (composerTitleEl) composerTitleEl.textContent = currentTab === "school" ? "New School Post" : "New Media Post";
  if (postTitleInput) postTitleInput.value = "";
  if (postTextInput) postTextInput.value = "";
  if (postFileInput) postFileInput.value = "";
  if (fileHint) fileHint.textContent = "Optional: attach image/PDF.";

  if (composerOverlay) composerOverlay.style.display = "grid";
...
postFileInput?.addEventListener("change", () => {
  ...
  if (fileHint) fileHint.textContent = f ? `Selected: ${f.name}` : "Optional: attach image/PDF.";
});
...
window.submitPost = async function () {
  ...
  try {
    if (file) {
      ...
      fileURL = up.url;
      fileType = up.type;
      if (fileHint) fileHint.textContent = "Uploaded ✔";
    }
...
function startRealtimePosts(section) {
  ...
  if (sectionBody) sectionBody.innerHTML = `<div class="empty">Loading...</div>`;
  postCache = {};
  ...
  unsubPosts = onSnapshot(
      ...
      for (const p of posts) {
        html += `
          <div class="card postCard" data-postid="${escapeHTML(p.id)}" style="padding:14px; background:rgba(255,255,255,.05); box-shadow:none;">
            ...
            ${
              p.text
                ? `<div style="color:rgba(234,234,255,.78); white-space:pre-wrap; line-height:1.5; max-height:4.2em; overflow:hidden;">${escapeHTML(
                    p.text
                  )}</div>`
                : ""
            }
            ${
              p.fileURL
                ? `<div style="margin-top:8px; color:rgba(234,234,255,.55); font-size:12px;">Attachment included • click to view</div>`
                : ""
            }
    ...
    (err) => {
      console.error(err);
      if (sectionBody) sectionBody.innerHTML = `<div class="empty">Error loading posts. Check Console.</div>`;
    }
  );
}
...
function startRealtimePendingCount(sectionOrAll) {
  ...
  unsubPendingCount = onSnapshot(qRef, (snap) => {
    if (!sideBody) return;
    if (sectionOrAll === "all") {
...
function startRealtimeAdminPanel() {
  if (!isAdmin()) {
    if (sectionBody) sectionBody.innerHTML = `<div class="empty">You are not an admin.</div>`;
    return;
  }
  ...
  if (sectionBody) sectionBody.innerHTML = `<div class="empty">Loading admin panel...</div>`;
                  ...
                  <div class="card" style="padding:14px; background:rgba(255,255,255,.05); box-shadow:none;">
                    <div style="display:flex; justify-content:space-between; gap:12px; align-items:center; flex-wrap:wrap;">
                      <div style="flex:1; min-width:260px;">
                        ...
                        ${
                          p.text
                            ? `<div style="margin-top:6px; color:rgba(234,234,255,.78); white-space:pre-wrap; line-height:1.5;">${escapeHTML(
                                p.text
                              )}</div>`
                            : ""
                        }
                ...
                <div style="display:flex; justify-content:space-between; gap:12px; align-items:center; flex-wrap:wrap;">
                  <div>
                    ...
                    <div style="color:rgba(234,234,255,.65); font-size:13px;">
                      ...
                      • UID: <span style="opacity:.6;">${escapeHTML(uid.slice(0, 6))}…</span>
                    </div>
                  </div>
